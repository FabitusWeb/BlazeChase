...
test('all 12 handcrafted layouts parse and are structurally valid', () => {
  const handcrafted = LAYOUTS.filter(l => !l.id.startsWith('ca-'));
  assert.strictEqual(handcrafted.length, 12);
...
test('layout difficulties: 4 EASY, 5 MEDIUM, 3 HARD (handcrafted)', () => {
  const count = { EASY: 0, MEDIUM: 0, HARD: 0 };
  for (const l of LAYOUTS.filter(l => !l.id.startsWith('ca-'))) count[l.difficulty]++;
  assert.deepStrictEqual(count, { EASY: 4, MEDIUM: 5, HARD: 3 });
});
...
test('arenaList() returns 12 handcrafted entries with unique ids and valid metadata', () => {
