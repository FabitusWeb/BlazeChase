...
const { generateArena, isSolid } = require('./arena');
...
const LAYOUTS = [
  ...
  // ONEWAY walls, OVERLAYS/GIZ/BGSTICKERS decorations.
  {
    ...
    difficulty: 'MEDIUM',
    theme: 'DESERT',
    // F7c: path vehicle — giro del corridoio alto (waypoints in tile coords)
    pathVehicles: [
      { points: [[1, 1], [38, 1], [38, 4], [1, 4]], speed: 60 },
    ],
    map: [
      '########################################',
      '#......................................#',
...
function parseArena(layout) {
  ...
  // Path vehicles (F7c, CA PATHVEHICLES): layout.pathVehicles =
  // [{ points: [[c,r],...], speed? }] — waypoints in tile coords, loop
  hazards.pathVehicles = (layout.pathVehicles || []).map((pv, i) => {
    if (!Array.isArray(pv.points) || pv.points.length < 2) {
      throw new Error(`Arena '${layout.id}': pathVehicle ${i} needs >= 2 waypoints`);
    }
    for (const [c, r] of pv.points) {
      if (r < 0 || r >= ROWS || c < 0 || c >= COLS || isSolid(tiles[r][c])) {
        throw new Error(`Arena '${layout.id}': pathVehicle ${i} waypoint ${c},${r} non calpestabile`);
      }
    }
    return {
      points: pv.points.map(([c, r]) => center(c, r)),
      speed:  pv.speed || CONFIG.HAZARDS.PATHVEHICLE.SPEED,
    };
  });
